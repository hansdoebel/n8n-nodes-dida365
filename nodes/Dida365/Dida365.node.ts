...
import { dida365ApiRequest, getProjects, getTasks } from './GenericFunctions';

import { taskFields, taskOperations, projectFields, projectOperations } from './descriptions';

export class Dida365 implements INodeType {
	methods = {
		loadOptions: {
	...
	description: INodeTypeDescription = {
		...
		version: 1,
		description: 'Dida365 is a powerful task management application',
		defaults: {
			name: 'Dida365',
		},
		inputs: ['main'],
			...
			if (resource === 'project') {
				...
				switch (operation) {
					...
					case 'create': {
						...
						responseData = await dida365ApiRequest.call(this, 'POST', endpoint, body);
						break;
					}
					//---------------------------------
					//	GET PROJECT
					...
					case 'get': {
						...
						responseData = await dida365ApiRequest.call(this, 'GET', endpoint);
						break;
					}
					//---------------------------------
					//	UPDATE PROJECT
					...
					case 'delete': {
						...
						const endpoint = `/open/v1/project/${projectId}`;
						responseData = await dida365ApiRequest.call(this, 'DELETE', endpoint);
						break;
					}
					default:
						throw new NodeOperationError(
				...
				switch (operation) {
					...
					case 'complete': {
						...
						responseData = await dida365ApiRequest.call(this, 'POST', endpoint, {});
						break;
					}
					//---------------------------------
					//	DELETE TASK
